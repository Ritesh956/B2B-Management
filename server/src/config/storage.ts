// Local-disk upload storage (multer). This was previously named s3.ts even
// though nothing here touches S3 — the name alone caused a production outage
// class of its own (everyone assumed uploads were durable; they're not).
//
// KNOWN LIMITATION: files live on the server's local disk. On Render's free
// tier that disk is ephemeral — every deploy/restart wipes uploads/. Durable
// storage needs a real object store (S3/R2) or a Render persistent disk;
// both require account setup outside this repo. Until then, PUBLIC_BASE_URL
// must point at the deployed API origin so the URLs written into the DB are
// at least reachable while the files exist.
import fs from 'fs';
